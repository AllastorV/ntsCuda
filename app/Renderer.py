...
class DefaultRenderer(AbstractRenderer):
    ...
    def set_up(self):
        ...
        if upscale_2x:
            # Ensure upscaled dimensions are divisible by 4
            uw = render_wh[0] * 2
            uh = render_wh[1] * 2
            if uw % 4 != 0:
                uw = uw - (uw % 4)
        ...
        self.config = Config(
            ...
            audio_process=getattr(self, 'audio_process', False),
            audio_sat_beforevol=getattr(self, 'audio_sat_beforevol', 4.5),
            audio_lowpass=getattr(self, 'audio_lowpass', 10896),
            audio_noise_volume=getattr(self, 'audio_noise_volume', 0.03),
        )
    ...
    def run(self):
        ...
        # BUG FIX: status_string initialised here so the pause branch can
        # reference it safely before the first frame is processed.
        status_string = 'Render starting...'
        ...
        audio_process = self.config.get('audio_process', False)
        audio_noise_volume = self.config.get('audio_noise_volume', 0.03)
        audio_sat_beforevol = self.config.get('audio_sat_beforevol', 4.5)
        audio_lowpass = self.config.get('audio_lowpass', 10896)
        ...
        if audio_process:
            ...
            aud_ff_noise = aud_ff_noise.filter('volume', audio_noise_volume)

            aud_ff_fx = final_audio.filter("volume", audio_sat_beforevol).filter("alimiter", limit="0.5").filter("volume", 0.8)
            aud_ff_fx = aud_ff_fx.filter("firequalizer", gain=f'if(lt(f,{audio_lowpass}), 0, -INF)')
